...
use crate::stage::{Consumer, PortGroup, Processor, Producer, Stage};
...
impl Pipeline {
    pub fn compile(
        ...
        for (idx, nodes) in chains.iter().enumerate() {
            ...
            let has_producer = nodes
                .first()
                .is_some_and(|&id| matches!(g.stage(id), Stage::Producer(_)));
            let has_consumer = nodes
                .last()
                .is_some_and(|&id| matches!(g.stage(id), Stage::Consumer(_)));

            let producer: Option<Box<dyn Producer>> = if has_producer {
                match g.take_stage(nodes[0]) {
                    Stage::Producer(p) => Some(p),
                    _ => unreachable!(),
                }
            } else {
                None
            ...
            let consumer: Option<Box<dyn Consumer>> = if has_consumer {
                match g.take_stage(*nodes.last().unwrap()) {
                    Stage::Consumer(c) => Some(c),
                    _ => unreachable!(),
                }
            } else {
                None
    ...
    for ei in g.edge_indices() {
        ...
        if ports.from_port as usize >= src_output_count {
            return Err(Error::internal(format!(
                "edge {} -> {}: from_port {} out of bounds ({} output ports)",
                g.node_kind(src),
                g.node_kind(dst),
                ports.from_port,
                src_output_count
        ...
        if ports.to_port as usize >= dst_input_count {
            return Err(Error::internal(format!(
                "edge {} -> {}: to_port {} out of bounds ({} input ports)",
                g.node_kind(src),
                g.node_kind(dst),
                ports.to_port,
                dst_input_count
...
fn outport_count(g: &ExecGraph, id: StageId, group: &PortGroup) -> usize {
    match group {
        ...
        PortGroup::Variable(_) => g
            .edges_out(id)
            .map(|e| e.ports.from_port as usize + 1)
            .max()
            .unwrap_or(0),
    }
}
...
fn inport_count(g: &ExecGraph, id: StageId, group: &PortGroup) -> usize {
    match group {
        ...
        PortGroup::Variable(_) => g
            .edges_in(id)
            .map(|e| e.ports.to_port as usize + 1)
            .max()
            .unwrap_or(0),
    }
}
...
fn insert_transfers(g: &mut ExecGraph, devs: &mut HashMap<StageId, Device>) {
    ...
    for (src, dst, ports) in edges {
        ...
        let is_transferrable = data_kind.is_some_and(|k| {
            k == crate::stage::DataKind::Tile || k == crate::stage::DataKind::Neighborhood
        });
        if sd != Device::Gpu && dd == Device::Gpu && is_transferrable {
            let mid = g.add_stage(Upload::stage());
            if let Some(ei) = g.find_edge(src, dst) {
                g.remove_edge(ei);
            }
            g.add_edge(
                src,
...
fn compute_work_total(g: &ExecGraph, order: &[StageId]) -> usize {
    ...
    for &id in order {
        let input_work = g
            .edges_in(id)
            .map(|e| output_work.get(&e.from).copied().unwrap_or(0))
            .max()
            .unwrap_or(0);
        let received = if g.edges_in(id).count() == 0 {
            1
        } else {
            input_work
        };
        total += received;
        let ow = if g.edges_in(id).count() == 0 {
            g.stage(id).source_items()
        } else {
            (input_work as f64 * g.stage(id).work_multiplier()).ceil() as usize
        };
