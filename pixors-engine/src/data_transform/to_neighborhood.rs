...
impl TileToNeighborhood {
    ...
    fn try_emit_gpu(
        ...
        for dy in -r..=r {
            for dx in -r..=r {
                let logical_tx = tx as i32 + dx;
                let logical_ty = ty as i32 + dy;
        ...
        for info in &tile_infos {
            // Re-clamp logical coordinates to find the physical tile in cache
            let logical_tx = info.px / self.tile_size as i32;
            let logical_ty = info.py / self.tile_size as i32;
            let gx = logical_tx.clamp(0, tiles_x - 1).max(0) as u32;
            let gy = logical_ty.clamp(0, tiles_y - 1).max(0) as u32;
            let nkey = TileGridPos {
                mip_level: mip,
