...
use crate::pipeline::sgraph::cache::CacheIndex;
use crate::pipeline::sgraph::graph::{NodeId, StateGraph};
use crate::pipeline::sgraph::node::{ExpandCtx, ExpansionOption, StateNodeTrait};

use crate::debug_stopwatch;
use crate::pipeline::egraph::graph::{ExecEdgePorts, ExecGraph, StageId};
use crate::pipeline::egraph::stage::{Device, ExecStage, Stage};
...
pub fn compile(
    ...
    let mut chosen_stages: HashMap<NodeId, Vec<ExecStage>> = HashMap::with_capacity(topo.len());
    ...
    for er in spec.graph.edge_references() {
        ...
        if from_dev == to_dev {
            ...
            let bridge = match (from_dev, to_dev) {
                (Device::Cpu, Device::Gpu) => ExecStage::Upload(exec::Upload),
                (Device::Gpu, Device::Cpu) => ExecStage::Download(exec::Download),
                _ => unreachable!(),
            };
            tracing::info!(
