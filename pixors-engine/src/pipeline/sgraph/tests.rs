#[cfg(test)]
mod tests {
    use crate::pipeline::sgraph::actions::{Action, ActionBatch};
    use crate::pipeline::sgraph::cache::{cache_key, CacheIndex};
    use crate::pipeline::sgraph::compile::{compile, ExecutionMode};
    use crate::pipeline::sgraph::graph::{EdgePorts, NodeId, StateGraph};
    use crate::pipeline::sgraph::history::History;
    ...
    fn action_insert_node_increases_version() {
        ...
        let batch: ActionBatch = vec![Action::InsertNode {
            node: StateNode::Export(state::Export {
                path: "out.png".into(),
                format: crate::pipeline::sgraph::node::ExportFormat::Png,
            }),
            assigned: None,
        }];
    ...
    fn pathbuilder_png_roundtrip() {
        ...
        use crate::pipeline::sgraph::node::ExportFormat;
        use crate::pipeline::sgraph::builder::PathBuilder;
    ...
    fn blur_4x4_rgba_correct() {
        ...
        let mut emitter = crate::pipeline::egraph::emitter::Emitter::new();
        use crate::pipeline::egraph::runner::OperationRunner;
        runner
            .process(crate::pipeline::egraph::item::Item::Neighborhood(nbhd), &mut emitter)
            .unwrap();
        let result = emitter.into_items();
        assert_eq!(result.len(), 1);
        let blurred = match &result[0] {
            crate::pipeline::egraph::item::Item::Tile(t) => t,
