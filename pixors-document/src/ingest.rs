...
/// Result of prepare_ingest. The graph must be consumed by run_graph.
/// On Done, caller pushes the session.
...
/// Prepare an ingest pipeline that decodes an image file and writes tiles to disk.
pub fn prepare_ingest(
    ...
    document.assets.primary_path = Some(path.to_path_buf());
