...
use crate::document::LayerFilter;
        ...
        if let Some(tab) = state.tab_mut(self.tab) {
            if let Some(preview) = tab.session.active_preview.take() {
                for (param, value) in &preview.overrides {
                    // Read the current (before) value from the document
                    ...
                    tab.history.push(Arc::new(mutation), &mut tab.document);
                }
