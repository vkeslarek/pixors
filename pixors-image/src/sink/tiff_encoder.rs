...
static TE_OUTPUTS: &[PortDeclaration] = &[];
static TE_PORTS: PortSpecification = PortSpecification {
    inputs: PortGroup::Fixed(TE_INPUTS),
    outputs: PortGroup::Fixed(TE_OUTPUTS),
};
...
impl Stage for TiffEncoderStage {
    ...
    fn consumer(&self) -> Option<Box<dyn Consumer>> {
        Some(Box::new(TiffEncoderConsumer::new(
            self.path.clone(),
            self.config.clone(),
            self.dpi,
...
impl Consumer for TiffEncoderConsumer {
