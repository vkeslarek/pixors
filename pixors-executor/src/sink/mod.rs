...
impl From<ViewportSink> for SinkNode {
    fn from(v: ViewportSink) -> Self {
        Self::Viewport(v)
    }
}

impl From<PngEncoder> for SinkNode {
    fn from(v: PngEncoder) -> Self {
        Self::PngEncoder(v)
    }
}

impl From<PngEncoderV2> for SinkNode {
    fn from(v: PngEncoderV2) -> Self {
        Self::PngEncoderV2(v)
    }
}

impl From<TiffEncoderStage> for SinkNode {
    fn from(v: TiffEncoderStage) -> Self {
        Self::TiffEncoder(v)
    }
}

impl From<CacheWriter> for SinkNode {
    fn from(v: CacheWriter) -> Self {
        Self::CacheWriter(v)
    }
}

impl From<ViewportCacheSink> for SinkNode {
    fn from(v: ViewportCacheSink) -> Self {
        Self::ViewportCacheSink(v)
    }
}
