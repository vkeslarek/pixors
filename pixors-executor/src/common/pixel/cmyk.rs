...
impl Pixel for Cmyk<f16> {
    fn unpack(self) -> [f32; 4] {
        [self.c.to_f32(), self.m.to_f32(), self.y.to_f32(), self.k.to_f32()]
    }
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self {
...
impl Pixel for Cmyk<f32> {
    fn unpack(self) -> [f32; 4] { [self.c, self.m, self.y, self.k] }
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self { c: rgba[0].clamp(0.0, 1.0), m: rgba[1].clamp(0.0, 1.0), y: rgba[2].clamp(0.0, 1.0), k: rgba[3].clamp(0.0, 1.0) }
    }
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        let r = rr.to_array(); let g = gg.to_array(); let b = bb.to_array(); let a = aa.to_array();
        for i in 0..4 {
            out[i] = Self { c: r[i].clamp(0.0,1.0), m: g[i].clamp(0.0,1.0), y: b[i].clamp(0.0,1.0), k: a[i].clamp(0.0,1.0) };
        }
    }
}
...
impl Pixel for CmykA<f16> {
    fn unpack(self) -> [f32; 4] { [self.c.to_f32(), self.m.to_f32(), self.y.to_f32(), self.k.to_f32()] }
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self {
            c: f16::from_f32(rgba[0].clamp(0.0, 1.0)),
            m: f16::from_f32(rgba[1].clamp(0.0, 1.0)),
    ...
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        ...
        for i in 0..4 {
            out[i] = Self {
                c: f16::from_f32(r[i].clamp(0.0,1.0)), m: f16::from_f32(g[i].clamp(0.0,1.0)),
                y: f16::from_f32(b[i].clamp(0.0,1.0)), k: f16::from_f32(a[i].clamp(0.0,1.0)),
                a: f16::ONE,
            };
        }
    }
...
impl Pixel for CmykA<f32> {
    fn unpack(self) -> [f32; 4] { [self.c, self.m, self.y, self.k] }
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self { c: rgba[0].clamp(0.0,1.0), m: rgba[1].clamp(0.0,1.0), y: rgba[2].clamp(0.0,1.0), k: rgba[3].clamp(0.0,1.0), a: 1.0 }
    }
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        let r=rr.to_array(); let g=gg.to_array(); let b=bb.to_array(); let a=aa.to_array();
        for i in 0..4 {
            out[i] = Self { c: r[i].clamp(0.0,1.0), m: g[i].clamp(0.0,1.0), y: b[i].clamp(0.0,1.0), k: a[i].clamp(0.0,1.0), a: 1.0 };
        }
    }
}
