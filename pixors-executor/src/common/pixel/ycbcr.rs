...
impl Pixel for YCbCr<u8> {
    ...
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self {
            y: (rgba[0].clamp(0.0, 255.0) + 0.5) as u8,
            cb: (rgba[1].clamp(0.0, 255.0) + 0.5) as u8,
            cr: (rgba[2].clamp(0.0, 255.0) + 0.5) as u8,
        }
    }
    ...
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, _aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        ...
        for i in 0..4 {
            out[i] = Self {
                y: (r[i].clamp(0.0, 255.0) + 0.5) as u8,
                cb: (g[i].clamp(0.0, 255.0) + 0.5) as u8,
                cr: (b[i].clamp(0.0, 255.0) + 0.5) as u8,
            };
        }
    }
}
