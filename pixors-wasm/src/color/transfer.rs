...
fn srgb_encode(y: f32) -> f32 {
    if y <= 0.0031308 { 12.92 * y } else { 1.055 * y.powf(1.0 / 2.4) - 0.055 }
}

fn rec709_decode(x: f32) -> f32 {
    if x < 0.081 { x / 4.5 } else { ((x + 0.099) / 1.099).powf(1.0 / 0.45) }
}
fn rec709_encode(y: f32) -> f32 {
    if y < 0.018 { 4.5 * y } else { 1.099 * y.powf(0.45) - 0.099 }
}

fn prophoto_decode(x: f32) -> f32 {
    if x <= 1.0 / 32.0 { x / 16.0 } else { x.powf(1.8) }
}
fn prophoto_encode(y: f32) -> f32 {
    if y <= 0.001953125 { 16.0 * y } else { y.powf(1.0 / 1.8) }
}
...
mod tests {
    ...
    #[wasm_bindgen_test] fn linear_inverse()   { assert_inverse(TransferFn::Linear); }
    #[wasm_bindgen_test] fn srgb_inverse()     { assert_inverse(TransferFn::SrgbGamma); }
    #[wasm_bindgen_test] fn rec709_inverse()   { assert_inverse(TransferFn::Rec709Gamma); }
    #[wasm_bindgen_test] fn gamma22_inverse()  { assert_inverse(TransferFn::Gamma22); }
    #[wasm_bindgen_test] fn gamma24_inverse()  { assert_inverse(TransferFn::Gamma24); }
    #[wasm_bindgen_test] fn gamma26_inverse()  { assert_inverse(TransferFn::Gamma26); }
    #[wasm_bindgen_test] fn prophoto_inverse() { assert_inverse(TransferFn::ProPhotoGamma); }
    #[wasm_bindgen_test] fn pq_inverse()       { assert_inverse(TransferFn::Pq); }
    #[wasm_bindgen_test] fn hlg_inverse()      { assert_inverse(TransferFn::Hlg); }
}
